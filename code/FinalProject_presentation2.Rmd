...
Scatterplot <- ggplot( Gene_data, aes(x = {{con.cov}}, y = gene_expression))+
  ...
  MyTheme+
  labs(title = substitute(paste("The association of ", italic(gene)," ", "expression and"," ", con.list)) ,x = substitute(paste(con.list," ","in yrs", " ", "(", "mean", "=", MeanCon,","," ", "median","=", MedianCon,")")), y = substitute(paste(italic(gene)," ", "expression")))+
...
Boxplot <- ggplot( Gene_data, aes(x = {{cat.cov1}}, y = gene_expression, fill = {{cat.cov2}}))+
  geom_boxplot() +
  MyTheme+
  scale_fill_manual(values = c("blue3","firebrick"),name = cat.list2, labels = c("No(75)", "Yes(51)"))+
  ...
  scale_x_discrete(name = cat.list1, labels =c( "Female(51)", "Male(74)", "Unknown(1)"))+
  theme(text = element_text(size = 10))

#Boxploy<- Box + str_to_title()
plot(Boxplot)
}
...
Con.cov_List2 = c("age","ferritin.ng.ml." )
...
Scatterplot <- ggplot( Gene_data, aes(x = {{con.cov}}, y = gene_expression))+
  geom_point(color = "red", alpha = .5, size = 3)+
  MyTheme+
  labs(title = substitute(paste("The association of ", italic(gene)," ", "expression and"," ", con.list)) ,x = substitute(paste(con.list," ", " ", "(", "mean", "=", MeanCon,","," ", "median","=", MedianCon,")")), y = substitute(paste(italic(gene)," ", "expression")))+
  theme(text = element_text(size = 11)) + 
  theme(plot.title = element_text(hjust = .35))
...
function_pres_newedited <- function(data, GeneList, Con.cov_List, cat.cov_List){
...
function_pres_newedited(LinkedData,GeneList2 , Con.cov_List2, cat.cov_List2)
